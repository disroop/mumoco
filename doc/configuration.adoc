...
The configuration of mumoco is set in a json file, default config-build.json. Therefore several configuration can be applied. Here is an example of a configuration.

[source,json]
----
include::example/config-build.json[]
----

Each parameter of the config-build.json file is explained in this table:
...
is configured, it is mandatory that a name and url is configured. By default a remote in the config-build.json file is always added to the top of the conan remote list.
